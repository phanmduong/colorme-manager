...
export const loadKPISettings = (token) => {
  const url =
    env.MANAGE_API_URL_2222 + '/v1/settings/by-group/kpi?token=' + token;
  return axios.get(url);
};

export const loadKPIEmployees = (token) => {
  const url = env.MANAGE_API_URL_2222 + '/v1/employees?token=' + token;
  return axios.get(url);
};

export const addKpis = (kpiData, token) => {
  const url = env.MANAGE_API_URL_2222 + '/v1/kpis?token=' + token;
...
export const loadCampaigns = (token) => {
  const url =
    env.MANAGE_API_URL_2222 + '/v1/marketing-campaigns?token=' + token;
  return axios.get(url);
};

export const loadSources = (token) => {
  const url = env.MANAGE_API_URL_2222 + '/v1/sources?token=' + token;
  return axios.get(url);
};

export const loadCourses = (token) => {
  const url = env.MANAGE_API_URL_2222 + '/v1/courses?status=1&token=' + token;
  return axios.get(url);
};
