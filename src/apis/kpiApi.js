...
export const loadKPISettings = (token, domain) => {
  const url =
    env.manageApiUrl2222(domain) + '/v1/settings/by-group/kpi?token=' + token;
  return axios.get(url);
};

export const loadKPIEmployees = (token, domain) => {
  const url = env.manageApiUrl2222(domain) + '/v1/employees?token=' + token;
  return axios.get(url);
};

export const addKpis = (kpiData, token, domain) => {
  const url = env.manageApiUrl2222(domain) + '/v1/kpis?token=' + token;
...
export const loadCampaigns = (token, domain) => {
  const url =
    env.manageApiUrl2222(domain) + '/v1/marketing-campaigns?token=' + token;
  return axios.get(url);
};

export const loadSources = (token, domain) => {
  const url = env.manageApiUrl2222(domain) + '/v1/sources?token=' + token;
  return axios.get(url);
};

export const loadCourses = (token, domain) => {
  const url = env.manageApiUrl2222(domain) + '/v1/courses?status=1&token=' + token;
  return axios.get(url);
};
