...
class CollectMoneyComponent extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      nextProps.isUpdatingMoneyStudent !== this.props.isUpdatingMoneyStudent
    ) {
  ...
  updateMoneyStudent() {
    if (this.props.formInfoMoney.code.trim().length <= 0) {
      Alert.alert('Thông báo', alert.MUST_ENTER_CODE);
      return;
    }
    if (this.props.formInfoMoney.money.trim().length <= 0) {
      Alert.alert('Thông báo', alert.MUST_ENTER_MONEY);
      return;
    }
          ...
          <View style={styles.containerModal}>
            {!this.state.isKeyboardShow ? (
              <View style={styles.containerInfoStudent}>
                <Thumbnail
                  large
                  source={{uri: this.state.student.avatar_url}}
                />
                <View style={styles.contentInfoStudent}>
                  <Text style={styles.nameStudent} maxLength={4}>
                    {this.state.student.name.length > 14
                      ? this.state.student.name
                          .substring(0, 14)
                          .trim()
                          .toUpperCase() + '...'
                      : this.state.student.name.trim().toUpperCase()}
                  </Text>
                  <Text style={styles.nameClass}>
                    {this.state.register.class.length > 17
                      ? this.state.register.class.substring(0, 18).trim() +
                        '...'
                      : this.state.register.class.trim()}
                  </Text>
                  <Call
                    url={'tel:' + this.state.student.phone}
                    phone={
                      this.state.student.phone.length > 17
                  ...
                  <Text style={styles.email}>
                    {this.state.student.email.length > 17
            ...
            <View style={{flex: 1}}>
              <View style={styles.containerCodeModal}>
                <View style={styles.contentCodeModal}>
                  <Text style={styles.titleCodeModal}>
                    Mã học viên tiếp theo:{' '}
                  </Text>
                  <Text style={styles.codeModal}>{this.props.nextCode}</Text>
                </View>
                <View style={styles.contentCodeModal}>
                  <Text style={styles.titleCodeModal}>
                    Mã học viên chờ tiếp theo:{' '}
                  </Text>
                  <Text style={styles.codeModal}>
                  ...
                  onPress={() => {
                    if (
                      ...
                      this.props.updateFormData(
                        'isReceivedCard',
                        !this.props.formInfoMoney.isReceivedCard,
                      );
                    }
                  }}
                />
              ...
              <InputGroup style={styles.inputGroup}>
                <Input
                  ...
                  onChangeText={data =>
                    this.props.updateFormData('money', data.replace(/,/g, ''))
                  }
                  style={styles.textInput}
                  ref="money"
                  returnKeyType={'next'}
