...
@observer
class HistoryMeetingComponent extends React.Component {
  ...
  handleRefresh = () => {
    this.props.store.refreshHistoryMeetingDetail();
  };

  render() {
    const {isLoading, meetingsHistory, refreshing} = this.props.store;
    return (
        ...
        {isLoading ? (
          ...
          <ScrollView
            ...
            {meetingsHistory.length > 0 && (
              <Section>
                <HeaderSection title={'Cuộc họp'} subtitle={'Đã diễn ra'} />
...
export default withStyle()(HistoryMeetingComponent);
