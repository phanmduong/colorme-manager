...
import {SafeAreaView, View, Text, ScrollView, TouchableOpacity, Dimensions, Alert} from 'react-native';
...
class StoreMeetingComponent extends React.Component {
    ...
    getFilterUsers = (users) => {
        ...
        if (departmentIds.length > 0) {
            users = users.filter(user => departmentIds.indexOf(user.department_id) >= 0)
        }
                    ...
                    <ScrollView>
                        ...
                        <View style={[{marginTop: 40}, styles.flexRow]}>
                            <View style={styles.flex1}>
                                ...
                                <TouchableOpacity style={[styles.textForm, {marginRight: 10}]}
                                                  onPress={this.openDatePicker}>
                                    <Text style={{
                                        fontSize: 15,
                                    }}>{meeting.date.format("DD/MM/YYYY")}</Text>
                                </TouchableOpacity>
                            </View>
                            <View style={styles.flex1}>
                                <Text style={styles.titleForm}>Giờ diễn ra</Text>
                                <TouchableOpacity style={styles.textForm} onPress={this.openTimePicker}>
                                    <Text style={{
                                        fontSize: 15,
                                    }}>{meeting.date.format("HH:mm")}</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
                        <View style={{marginTop: 40}}>
                            ...
                            <Picker
                                ...
                                {filter.rooms && filter.rooms.map(function (room, index) {
                                    return (<Item label={room.name} value={room.id} key={index}/>)
                                })}
                            </Picker>
                        </View>
                        <View style={{marginTop: 40}}>
                            ...
                            <View style={styles.containerTag}>
                                {filter.provinces && filter.provinces.map(function (province, index) {
                        ...
                        <View style={{marginTop: 40}}>
                            ...
                            <View style={styles.containerTag}>
                                {filter.departments && filter.departments.map(function (department, index) {
                        ...
                        <TouchableOpacity onPress={() => isStoring ? null : this.submitStoreMeeting()}>
                            <LinearGradient colors={['#E26800', '#E00000']}
                                            start={{x: 0, y: 0}} end={{x: 1, y: 0}}
