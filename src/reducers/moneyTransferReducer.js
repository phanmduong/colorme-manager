...
export default function moneyTransferReducer(
  state = initialState.moneyTransfer,
  action,
) {
  switch (action.type) {
    ...
    case types.LOAD_DATA_STAFF_OF_MONEY_TRANSFER_SUCCESSFUL:
      let staffListData =
        action.currentPageAll === 1
          ? action.staffListData
          : [...state.staffListData, ...action.staffListData];
      return Object.assign({}, state, {
        isLoadingStaffList: action.isLoadingStaffList,
        errorStaffList: action.errorStaffList,
        staffListData: updateStaffListDataData(
          staffListData,
    ...
    case types.BEGIN_CONFIRM_TRANSACTION:
      return {
        ...state,
        ...{
          transactionListData: updateTransactionListData(
            state.transactionListData,
            action.isLoadingConfirmTransaction,
            action.errorConfirmTransaction,
          ),
        },
      };
...
function updateTransactionListData(
  transactionListData,
  isLoadingConfirmTransaction,
  errorConfirmTransaction,
) {
